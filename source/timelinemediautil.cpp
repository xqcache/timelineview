...
QList<QImage> TimelineMediaUtil::loadThumbnails(const QString& path, int height, int step)
{
    AVFormatContext* fmt_ctx = nullptr;
    if (avformat_open_input(&fmt_ctx, path.toStdString().c_str(), nullptr, nullptr) != 0) {
        TL_LOG_ERROR("Failed to open media file: {}", path.toStdString());
    ...
    SwsContext* sws_ctx = sws_getContext(codec_ctx->width, codec_ctx->height, codec_ctx->pix_fmt, codec_ctx->width, codec_ctx->height, AV_PIX_FMT_RGB24,
        ...
        if (packet->stream_index == video_stream_idx) {
            ...
            while (avcodec_receive_frame(codec_ctx, frame) == 0) {
                if (frame_count % step == 0) {
                    ...
                    QImage img(rgb_frame->data[0], codec_ctx->width, codec_ctx->height, rgb_frame->linesize[0], QImage::Format_RGB888);
                    thumbnails.append(img.scaledToHeight(height));
                }
                frame_count++;
            }
        }
